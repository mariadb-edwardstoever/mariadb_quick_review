...
CREATE TABLE `SERVER_PERFORMANCE` (
  ...
  `INNODB_ROWS_DELETED` bigint(20) DEFAULT NULL,
  `INNODB_ROWS_INSERTED` bigint(20) DEFAULT NULL,
  `INNODB_ROWS_READ` bigint(20) DEFAULT NULL,
  `INNODB_ROWS_UPDATED` bigint(20) DEFAULT NULL,
  PRIMARY KEY (`ID`)
...
CREATE TABLE `EVENTS_WAITS_SUMMARY_BY_USER_BY_EVENT_NAME` (
  `RUN_ID` varchar(16) DEFAULT NULL,
  `USER` char(128) CHARACTER SET utf8mb3 COLLATE utf8mb3_bin DEFAULT NULL,
  `EVENT_NAME` varchar(128) NOT NULL,
  `COUNT_STAR` bigint(20) unsigned NOT NULL,
  `SUM_TIMER_WAIT` bigint(20) unsigned NOT NULL,
  `MIN_TIMER_WAIT` bigint(20) unsigned NOT NULL,
...
CREATE TABLE `ACCOUNTS` (
  `RUN_ID` varchar(16) DEFAULT NULL,
  `USER` char(128) CHARACTER SET utf8mb3 COLLATE utf8mb3_bin DEFAULT NULL,
  `HOST` char(255) CHARACTER SET utf8mb3 COLLATE utf8mb3_bin DEFAULT NULL,
...
CREATE VIEW IF NOT EXISTS V_WAITS_BY_LATENCY AS
SELECT RUN_ID,  EVENT_NAME, COUNT_STAR, SUM_TIMER_WAIT, AVG_TIMER_WAIT, MAX_TIMER_WAIT 
FROM EVENTS_WAITS_SUMMARY_GLOBAL_BY_EVENT_NAME 
WHERE EVENT_NAME != 'IDLE';


CREATE VIEW IF NOT EXISTS V_USERS_BY_IO_LATENCY AS 
SELECT RUN_ID,    USER, SUM(COUNT_STAR) AS COUNT,
...
CREATE VIEW IF NOT EXISTS USERS_BY_STATEMENT_LATENCY AS
SELECT RUN_ID,  USER, SUM(COUNT_STAR) AS TOTAL_STATEMENTS,
...
CREATE VIEW IF NOT EXISTS V_FILES_BY_TOTAL_IO AS 
SELECT RUN_ID,   FILE_NAME, COUNT_READ,
...
FORMAT(SUM_NUMBER_OF_BYTES_READ+SUM_NUMBER_OF_BYTES_WRITE,0) AS TOTAL, 
...
CREATE VIEW IF NOT EXISTS V_USERS_BY_CONNECTION AS
SELECT RUN_ID,  USER,  HOST, CURRENT_CONNECTIONS, TOTAL_CONNECTIONS
FROM ACCOUNTS 
WHERE USER IS NOT NULL;

CREATE VIEW IF NOT EXISTS V_TABLES_BY_STATEMENT_TYPE AS
SELECT RUN_ID,  
OBJECT_SCHEMA AS DB, 
OBJECT_NAME AS TBL, 
...
CREATE VIEW IF NOT EXISTS V_TABLES_WITH_FULL_TABLE_SCANS AS 
SELECT RUN_ID,  OBJECT_SCHEMA, OBJECT_NAME, 
FORMAT(COUNT_READ,0) AS ROWS_FULL_SCANNED 
FROM TABLE_IO_WAITS_SUMMARY_BY_INDEX_USAGE 
WHERE INDEX_NAME IS NULL 
AND COUNT_READ > 0;

CREATE VIEW IF NOT EXISTS V_TABLES_WITH_UNUSED_INDEXES AS 
SELECT RUN_ID,  OBJECT_SCHEMA, OBJECT_NAME, INDEX_NAME
FROM TABLE_IO_WAITS_SUMMARY_BY_INDEX_USAGE
